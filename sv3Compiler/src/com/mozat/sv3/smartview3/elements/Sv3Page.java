...
import com.mozat.sv3.smartview3.utils.IEventHandler;
...
public class Sv3Page implements IEventHandler {
	...
	public Rect getScrollRect(int canvasWidth, int canvasHeight) {
		...
		Rect r = root.getRect();
		result.x = (short) (canvasWidth - 7);
		result.width = 4;
		if (canvasHeight < r.height) {
			result.y = scrollY * canvasHeight / r.height;
			result.height = canvasHeight * canvasHeight / r.height;
			if (result.height < 3) {
				result.height = 3;
			}

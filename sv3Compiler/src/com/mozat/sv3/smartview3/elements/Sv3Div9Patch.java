...
public class Sv3Div9Patch extends Sv3Div implements IImageRequester {
			...
			if (image != null) {
				...
				if (w1 > 0) {
					for (int x = x1; x < rw - w2; x += w1) {
						ctx.clipRect(rx + x, ry, w1, h0);
						ctx.drawImage(image, rx + x - w0, ry);
						ctx.setClip(cx, cy, cw, ch);

						ctx.clipRect(rx + x, ry + rh - h2, w1, h2);
				...
				if (h1 > 0) {
					for (int y = y1; y < rh - h2; y += h1) {
						ctx.clipRect(rx, ry + y, w0, h1);
						ctx.drawImage(image, rx, ry + y - h0);
						ctx.setClip(cx, cy, cw, ch);

						ctx.clipRect(rx + rw - w2, ry + y, w2, h1);
	...
	public void didReceiveImage(Object image, String src) {
		boolean fire = false;
		if (src.startsWith(imageUrl)) {
			if (this.image != image) {
				this.image = image;
				fire = true;
			}
		} else if (src.startsWith(focusImageUrl)) {

...
public class MoWebUtil
{
	...
	public static interface MoWebListener
	{
		void onComplete(byte[] bytes, RawRequest request);
	}
	...
	public static void sendRawRequest(RawRequest request, boolean moagentHttp)
	{
		if (moagentHttp)
		{
			try
			{
				...
				OutMoWebMessage msg = new OutMoWebMessage(Adaptor.getInstance().browserServiceId, "test", bos.toByteArray());
				sendMessage(msg.toBytes(), Adaptor.getInstance().moagentWap, request);
		...
		} else
		{
			try
			{
				...
				{
					sendMessage(request.value.getBytes("UTF-8"), request.url, request);
				} else
				{
			...
			{
				MilkTaskImpl task = new MilkTaskImpl(new MoWebTaskRunner(request));
				...
				// }
				// }, 100);
				UIHelper.milk.timer.schedule(task.task, 100);
			}
		}
	}
	...
	private static class SendTask implements Runnable
	{
		...
		public void run()
		{
			...
			try
			{
				c = UIHelper.milk.getHttpConnection(url);

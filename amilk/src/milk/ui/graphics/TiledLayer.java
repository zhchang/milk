...
public class TiledLayer extends Layer {
	...
	private String tag=TiledLayer.class.getName();
	/**
...
//		tileWidth=(int)(tileWidth*MilkApp.milk.widthPer);
//		tileHeight=(int)(tileHeight*MilkApp.milk.heightPer);
		
		
//		Log.e(tag, "image width"+image.getWidth()+" height "+image.getHeight());
//		Log.e(tag, String.format("columns %d rows %d tileWidth %d tileHeight %d ",columns,rows,tileWidth,tileHeight));
		
//		if(UIHelper.milk.isScale)
//		{
////			tileWidth=(int)(tileWidth*UIHelper.milk.widthPer);
////			tileHeight=(int)(tileHeight*UIHelper.milk.heightPer);
//		}
//		
//		Log.e(tag, String.format("columns %d rows %d tileWidth %d tileHeight %d ",columns,rows,tileWidth,tileHeight));
...
//		int noOfFrames=rows*columns;
	...
	{
		return String.format("cellHeight %d cellWidth %d rows %d columns %d cellmatrix length %d", cellHeight,cellWidth,rows,columns,cellMatrix.length);
	}
...
//			if(tileCanvas==null)
//			{
//				tileBitmap=Bitmap.createBitmap(columns*getCellWidth(), rows*getCellWidth(), Config.ARGB_8888);
...
//					isUpdate=true;
//					drawImage(tileCanvas, tx, ty, sourceImage, tileSetX[tileIndex],
...
//			if(isUpdate)
//			{
//				resizeImage=Image.getResizeImage(tileBitmap);
//			}
//			
//			canvas.drawBitmap(resizeImage, this.x, this.y, null);
...
//		Rect rect_src = new Rect();
		Rect rect_src =RectPool.getRect();
		rect_src.left = sx;
		rect_src.right = sx + w;
		rect_src.top = sy;
...
//		Rect rect_dst = new Rect();
		Rect rect_dst=RectPool.getRect();
		rect_dst.left = x;
		rect_dst.right = x + w;
		rect_dst.top = y;
		rect_dst.bottom = y + h;
...
//		rect_src = null;
//		rect_dst = null;
		RectPool.recovery(rect_src);
		RectPool.recovery(rect_dst);
	}

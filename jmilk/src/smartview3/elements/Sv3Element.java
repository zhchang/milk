...
public class Sv3Element {
	...
	// LayoutContext subctx;

	// protected void initSubctx(int w, int h) {
	// if (subctx == null) {
	// subctx = new LayoutContext(w, h);
	// } else {
	// subctx.reset(w, h);
	// }
	// }
	...
	private String tips;
	public TextSegment tipsSegment;
	private String name;
	protected byte lineWrap = LINE_WRAP_NONE; // none, before, after, both
	private byte fontSize = FONT_SIZE_S; // small, medium, large
	...
	private long tipsDrawStartTime = 0;

	public long getTipsDrawStartTime() {
		return tipsDrawStartTime;
	}

	public void setTipsDrawStartTime(long value) {
		this.tipsDrawStartTime = value;
	}
	...
	protected void layoutContent(Rect newRect, LayoutContext ctx) {
		...
		// System.out.println("--------layoutContent newRect.height:"+newRect.height);
	}
	...
	protected static int layoutSingleLineText(Rect newRect, String text,
			LayoutContext ctx, MilkFont font, short[] inPadding, int marginX,
			int marginY, int minTextWidth) {
		int fontHeight = font.getHeight();
		// System.out.println("--------layoutSingleLineText fontheight:"
		// + fontHeight);
		int width = 0;
		if (text != null) {
	...
	protected static void renderSingleLineText(IRenderContext ctx, String text,
			short textWidth, Rect textBound, MilkFont font, int color,
			byte align) {
		if (text != null) {
			int fontheight = font.getHeight();
			// System.out.println("--------renderSingleLineText fontheight:"
			// + fontheight);
	...
	public void layout(LayoutContext ctx) {
		if (hidden) {
			...
			this.layoutContent(newRect, ctx);
	...
	private MilkFont lastFont = null;

	public final MilkFont getFont() {
		if (lastFont == null) {
			lastFont = getFont(fontStyle, fontSize);
		}
		// if (lastFont == null) {
		// throw new IllegalArgumentException(
		// "Sv3Element getFont() = null fontStyle:" + fontStyle
		// + "/fontSize:" + fontSize);
		// }
		return lastFont;
	}

	private MilkFont getFont(byte fontStyle, byte fontSize) {
		int size;
	...
	public boolean canFocus() {
		return url != null && url.length() > 0 && !isDisabledByParent()
				&& !isHidden() && !isHiddenByParent();
	}
	...
	protected void readAttrFromMobon(MobonReader r, int key)
		...
		case TAG_Confirmation:
			...
			break;
		case TAG_Tips:
			this.setTips(r.readStringOrNull());
			break;
		case TAG_Name:
	...
	public boolean setIntAttrib(String key, int value) {
		...
		if ("border".equals(key)) {
			...
			needsRelayout = true;
		} else if ("focused".equals(key)) {
			if (canFocus()) {
				this.setFocusedWithEvent(value != 0);
			}
		} else {
			return false;
		}
	...
	protected void fireAttribEvent(final String key,
			final boolean needsRepaint, final boolean needsRelayout) {
		if (page != null) {

			page.didChangeAttrib(Sv3Element.this, key, needsRepaint,
					needsRelayout);
	...
	public Object getAttrib(String key) {
		...
		if ("id".equals(key)) {
			...
			return new Integer(focused ? 1 : 0);
		}
		return null;
	}
	...
	public void setTips(String tips) {
		...
		if (tips != null && tips.length() > 0) {
			try {
				Vector pieces = VectorPool.produce();
				MilkFont tipFont = Adaptor.uiFactory.getFont(
						MilkFont.STYLE_PLAIN, MilkFont.SIZE_SMALL);
				new TextBreaker().breakText(tips, pieces, tipFont,
						Adaptor.milk.getCanvasWidth(),
						(short) Adaptor.milk.getCanvasWidth(), 1, null);
				tipsSegment = (TextSegment) pieces.elementAt(0);
				VectorPool.recycle(pieces);
			} catch (Exception t) {
				Adaptor.exception(t);
			}
		} else {
	...
	private final void setFontSize(byte fontSize) {
	...
	private final void setFontSize(String fontSize) {
	...
	private final void setFontStyle(byte fontStyle) {

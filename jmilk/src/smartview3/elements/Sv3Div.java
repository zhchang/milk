...
public class Sv3Div extends Sv3Element {
	...
	protected static Rect paddedRect = new Rect();
	...
	protected void clearMem() {
		...
		if (children != null) {
			...
			for (int i = 0; i < count; i++) {
				Sv3Element ele = (Sv3Element) children.elementAt(i);
				ele.clearMem();
				ele = null;
			}
	...
	public Sv3Element hit(int pointerX, int pointerY) {
		if (rect != null && rect.contains(pointerX, pointerY)) {
			...
			for (int i = size - 1; i >= 0; --i) {
				...
				if (result != null && result.canFocus()) {
					return result;
				}
			}
			if (this.canFocus()) {
	...
	protected void layoutContent(Rect newRect, LayoutContext ctx) {
		...
		// System.out.println("-----Sv3Div---layoutContent newRect.height:"+newRect.height);
	}
	...
	public Rect getPaddedRect() {
		paddedRect.copy(rect);
		paddedRect.x += resolvedPadding[1];
		paddedRect.y += resolvedPadding[0];
		paddedRect.width += resolvedPadding[1] + resolvedPadding[3];
		paddedRect.height += resolvedPadding[0] + resolvedPadding[2];
		return paddedRect;
	}

...
public class Sv3Button extends Sv3Element {
	...
	protected void readAttrFromMobon(MobonReader r, int key) throws IOException, MobonException {
	...
	protected void layoutContent(Rect newRect, LayoutContext ctx) {
		...
		titleWidth = (short) layoutSingleLineText(newRect, text, ctx, font, resolvedPadding, MARGIN_X, MARGIN_Y,
				MIN_WIDTH);
	}
	...
	public void render(IRenderContext ctx) {
		if (!hidden && rect != null) {
			...
			if (absRect.overlaps(viewport)) {
				Rect absPadded = absRect.toRectWithPadding(resolvedPadding, ctx.getPaddedRect());

				this.renderBg(ctx, bounding, radius, 0xffffff);

				// Rect textBound = new Rect(absPadded.x + MARGIN_X, absPadded.y + MARGIN_Y, absPadded.width - MARGIN_X
				// * 2, absPadded.height - MARGIN_Y * 2);
				Rect textBound = ctx.getTextBoundRect();
				textBound.copy(absPadded.x + MARGIN_X, absPadded.y + MARGIN_Y, absPadded.width - MARGIN_X * 2,
						absPadded.height - MARGIN_Y * 2);

				MilkFont font = this.getFont();
				renderSingleLineText(ctx, text, titleWidth, textBound, font, getResolvedColor(), Sv3Div.ALIGN_C);
			}
		}
	}

...
public class TextBreaker {
	...
	static final byte TOKEN_LANG_INVALID = 0, TOKEN_LANG_ENGLISH = 1, TOKEN_LANG_EASTASIA = 2, TOKEN_LANG_ARABIC = 3,
			TOKEN_LANG_OTHER = 4;
	...
	static boolean MOIsCharBreakableAfter(char c) {
		...
		return MOIsCharEnglishAndBreakableAfter(c) || MOIsCharOtherLanguageBreakableAfter(c);
	}
	...
	/*
	 * ! what is considered a TOKEN? A token should be one of the following: a smiley strings that matches any of the
	 * smiley strings a text token, which is a sequence of characters that fullfil the following readable characters
	 * seperated by white space (OR EOF) on both side character of the same language
	 */
	private Token nextToken() {
		...
		// boolean isFirstChar;
		char nextChar = 0;
		while (cursor < contentLength_ && cursor - currentIndex_ < MAX_TOKEN_LENGTH) // play safe, we don't allow a
																						// token to grow to a indefinate
																						// size
		{
			...
			if (isNewLine(curChar)) {
					...
					for (int i = 0; i < emoCount; ++i) {
						...
						if (tailLen >= 0) {
							...
							if (emoCursor - currentIndex_ == 0) {
								token.set(currentIndex_, cursor - currentIndex_, false, emo);
							} else {
								token.set(currentIndex_, emoCursor - currentIndex_, false);
								nextToken = new Token();
								nextToken.set(emoCursor, cursor - emoCursor, false, emo);
							}
							break;
						}
					}
				...
				if (isEmoticon) {
					...
					if (MOIsCharArabicOrHebrew(curChar)) {
						...
						canBreakAfterChar = MOIsCharEastAsiaAndBreakableAfter(curChar, nextChar);
					} else {
					...
					if (tokenLanguage != charLanguage) {
						// the current character is NOT included as part of the token
						--cursor; // we need to put back the character and leave it for the next token
						token.set(currentIndex_, cursor - currentIndex_, false);
						break;
					}

					if (canBreakAfterChar) {
						// the breakable character is included as the last char of the token
						token.set(currentIndex_, cursor - currentIndex_, false);
						break;
					}
				}
		...
		if (cursor > currentIndex_ && token.length == 0) // last segment before EOF
		{
			token.set(currentIndex_, cursor - currentIndex_, false);
		}
	...
	Token partOfToken(Token token, int tokenWidth, int fitWidth, int[] actualWidth) {
		actualWidth[0] = tokenWidth;
		if (tokenWidth < fitWidth) {
			return new Token(token);
		} else {
			Token partialToken = new Token();
			int partialLength = token.length * fitWidth / tokenWidth / 2; // start from a portion that is very unlikely
																			// to be longer than a line

			int widthOfPartialToken = font_.substringWidth(content_, token.start, partialLength);
			actualWidth[0] = widthOfPartialToken;

			while (partialLength < token.length && widthOfPartialToken < fitWidth) // keep shrinking until it fits
			{
				++partialLength;
				actualWidth[0] = widthOfPartialToken;
				widthOfPartialToken = font_.substringWidth(content_, token.start, partialLength);
			}
	...
	public void breakIt() {
		...
		while (token.length > 0) {
			// NSLog(@"(%d, %d, %x, %d)", token.start, token.length, token.flags, token.emoticon != nil);
			int currentLineMaxWidth = lineIndex == 0 ? firstLineWidth_ : clipWidth_;

			int tokenWidth = 0;
			if (token.emoticon == null) {
				tokenWidth = font_.substringWidth( content_, token.start, token.length);
			} else {
				tokenWidth = token.emoticon.getWidth() + token.emoticon.getXSpacing() * 2;
			}

			if (tokenWidth > clipWidth_) // if the token doesn't fit the full line, break it
			{
				if (clipWidth_ < MIN_REASONABLE_BOUND_WIDTH) // too small to be reasonable
				{
					...
					if (widthLeft > clipWidth_ / 2) // width left is more than half of full line width
					{
						int[] actualWidth = new int[1];
						Token partialToken = this.partOfToken(token, tokenWidth, widthLeft, actualWidth);

						this.addSegment(new TextSegment(partialToken.start, partialToken.length, lineIndex,
								actualWidth[0]));
	...
	public void breakText(String content, Vector pieces, MilkFont font, int firstLineWidth,
			short clipWidth, int maxLines, Hashtable emoticonTable) {

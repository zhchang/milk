...
//import milk.ui2.NativeInput;

public class BBRoom implements BBSelectBoxListener {
	...
	private BBInputBox inputBox;
	private BBSelectBox selectBox;

	private Object focusObject;
	public static BBFaceScreen bbFace;
	...
	public BBRoom(MilkApp app, byte type) {
		factory = app;
		...
		case Def.CHAT_TYPE_PRIVATE:
			inputBox = new BBInputBox(factory, roomType);
			selectBox = new BBSelectBox(factory);
			break;
		...
		if (bbFace == null) {
			bbFace = BBHallScreen.faceScreen;
		}
	...
	public void initRoom(int x, int y, int w, int h, int inputHeight) {
		...
		if (selectBox != null) {
			...
			selectBox.initSelectBox(x + w - BBSelectBox.DEFAULT_FRAME_WIDTH,
					selectY, BBSelectBox.DEFAULT_FRAME_WIDTH, inputBoxHeight);
		...
		if (bbFace!=null)
			bbFace.setPopScreenSizeForBB(roomRectW, roomRectH);
	...
	public void showNotify() {
		...
		if (bbFace != null) {
			if (roomType == Def.CHAT_TYPE_WORLD)
				bbFace.setPopScreenSizeForBB(roomRectW,
						roomRectH + Message.getTopMessageHeight());
			else {
				bbFace.setPopScreenSizeForBB(roomRectW, roomRectH);
			}
		}
	...
	boolean lockChatHallScreen() {
		if (focusObject instanceof BBSelectBox) {
			return ((BBSelectBox) focusObject).isShownSelectBox();
		}
		if (bbFace.isPopup()) {
			return true;
		}
	...
	boolean doRightSoftKey() {
		...
		if (focusObject instanceof BBSelectBox) {
			if (((BBSelectBox) focusObject).doRightSoftKey()) {
				return true;
			}
		}
		return false;
	...
	void handleKeyEvent(MKeyEvent key) {
		if (focusObject instanceof BBSelectBox) {
			BBSelectBox box = (BBSelectBox) focusObject;
...
//				break;
	...
	private boolean keyFire() {
		if (focusObject instanceof BBInputBox) {
			inputBox.setTarget(currentChatUserId, currentChatUserName);
			return false;
		} else if (focusObject instanceof BBSelectBox) {
			((BBSelectBox) focusObject).keyFire();
			...
			if (message.getCreateTime() != 0) {
				...
				if (menu != null) {
					...
					chatListener.notifyShowPopMenu(Def.popTitleWantTo, menu,
							toId, toName, null);
				}
			}
		}
...
//	private MArray array = new MArray();
	...
	void update() {
		...
		} else if (focusObject instanceof BBInputBox) {
			factory.getMilkCanvas().setCallSuperEvent(false);
		} else {
			factory.getMilkCanvas().setCallSuperEvent(true);
		}
	}
	...
	private void setCurrenFocusObject(Object newFocus) {
		if (newFocus instanceof BBInputBox) {
			((BBInputBox) newFocus).setFocus(true);
			if (!(focusObject instanceof BBInputBox) && focusObject != null)
				inputBox.showEdit();
			if (selectBox != null)
				selectBox.setFocus(false);
		} else if (newFocus instanceof BBSelectBox) {
			((BBSelectBox) newFocus).setFocus(true);

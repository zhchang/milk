...
public class ImageManipulator {
	 ...
	 */
	private void setTransform(int[] matrix, int cos, int sin, boolean mirrorX,
			boolean mirrorY) {
	...
	public void applyTransformation() {
		...
		long rotation;
		for (int i = 0; i < 4; i++) {
			rotation = VecMath.multiplyPoint(transformMatrix, 0, bitmapXPts[i],
					bitmapYPts[i]);
		...
		if (!orthogonal) {
			...
			rotation = VecMath.multiplyPoint(transformMatrix, 0, textureX,
					textureY);
			...
			// on all sides
			resultantScaleX = Fixed32.div((bitmapRight + Fixed32.toFP(4)),
					Fixed32.toFP(bitmap.getWidth()));
			resultantScaleY = Fixed32.div((bitmapBottom + Fixed32.toFP(4)),
					Fixed32.toFP(bitmap.getHeight()));
		...
		transformedRegion.set(Fixed32.toRoundedInt(maxX - minX) + 1,
				Fixed32.toRoundedInt(maxY - minY) + 1);
	 ...
	 * @since 1.1
	 */
	private void paintTransformedBitmap(Graphics g, int textureOriginX,
			int textureOriginY) {
		...
		g.drawTexturedPath(bitmapXPts, bitmapYPts, null, null, textureOriginX,
				textureOriginY, dux, dvx, duy, dvy, bitmap);
	}
	...
	public Bitmap transformAndPaintBitmap() {
		...
		Bitmap transformedBitmap = new Bitmap(bitmap.getType(),
				transformedRegion.width, transformedRegion.height);
	...
	public void scaleInto(Bitmap dst, int filterType) {
		scaleInto(0, 0, bitmap.getWidth(), bitmap.getHeight(), dst, 0, 0,
				dst.getWidth(), dst.getHeight(), filterType);
	}
	...
	public void scaleInto(Bitmap dst, int filterType, int iAspectRatioOption) {
		...
		case SCALE_TO_FILL:
			...
			if (scaleVert > scaleHoriz) {
				...
				srcShiftX = Fixed32.toRoundedInt(Fixed32.mul(
						(srcWidth - srcRegionWidth) >> 1, scaleVert));
				scaleInto(srcShiftX, 0, Fixed32.toRoundedInt(srcRegionWidth),
						bitmap.getHeight(), dst, 0, 0, dst.getWidth(),
						dst.getHeight(), filterType);
			} else if (scaleHoriz > scaleVert) {
				// source width fills destination
				int srcRegionHeight = Fixed32.div(dstHeight, scaleHoriz);
				srcShiftY = Fixed32.toRoundedInt(Fixed32.mul(
						(srcHeight - srcRegionHeight) >> 1, scaleVert));
				scaleInto(0, srcShiftY, bitmap.getWidth(),
						Fixed32.toRoundedInt(srcRegionHeight), dst, 0, 0,
						dst.getWidth(), dst.getHeight(), filterType);
			} else {
				// Both bitmaps have the same aspect ratio
				scaleInto(0, 0, bitmap.getWidth(), bitmap.getHeight(), dst, 0,
						0, dst.getWidth(), dst.getHeight(), filterType);
			}
			break;
		case SCALE_TO_FIT:
			/*
			...
			if (scaleVert < scaleHoriz) {
				...
				scaleInto(0, 0, bitmap.getWidth(), bitmap.getHeight(), dst,
						dstShiftX, 0, Fixed32.toRoundedInt(scaledWidth),
						dst.getHeight(), filterType);
			} else if (scaleHoriz < scaleVert) {
				...
				scaleInto(0, 0, bitmap.getWidth(), bitmap.getHeight(), dst, 0,
						dstShiftY, dst.getWidth(),
						Fixed32.toRoundedInt(scaledHeight), filterType);
			} else {
				// Both bitmaps have the same aspect ratio
				scaleInto(0, 0, bitmap.getWidth(), bitmap.getHeight(), dst, 0,
						0, dst.getWidth(), dst.getHeight(), filterType);
	 ...
	 * @since 1.1
	 */
	public void scaleInto(int srcLeft, int srcTop, int srcWidth, int srcHeight,
			Bitmap dst, int dstLeft, int dstTop, int dstWidth, int dstHeight,
			int filterType) {

		// Maintain same interface as 5.0 API
		if (dst == null) {
			throw new NullPointerException(
					"Destination bitmap can not be set to NULL");
		}
		if (!dst.isWritable()) {
			throw new IllegalArgumentException(
					"Destination bitmap should not be read-only");
		}
		...
		bitmapXPts = new int[] { dstLeft, dstLeft, dstLeft + dstWidth,
				dstLeft + dstWidth };
		bitmapYPts = new int[] { dstTop, dstTop + dstHeight,
				dstTop + dstHeight, dstTop };

		// Calculate the new scale based on the region sizes
		resultantScaleX = Fixed32.div(Fixed32.toFP(dstWidth),
				Fixed32.toFP(srcWidth));
		resultantScaleY = Fixed32.div(Fixed32.toFP(dstHeight),
				Fixed32.toFP(srcHeight));
	...
	public static Image convert(Bitmap bitmap) {
		...
		bitmap.getARGB(data, 0, bitmap.getWidth(), 0, 0, bitmap.getWidth(),
				bitmap.getHeight());
		return Image.createRGBImage(data, bitmap.getWidth(),
